...
class SphericalVisionSensor(Object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._planar_to_radial_depth_scalars = (
            self._get_planar_to_radial_depth_scalars()
        )
    ...
    def _assert_matching_resolutions(self):
        assert np.array_equal(
            self._sensor_depth.get_resolution(), self._sensor_rgb.get_resolution()
        )
    ...
    def _assert_matching_render_modes(self):
        assert (
            self._sensor_depth.get_render_mode() == self._sensor_rgb.get_render_mode()
        )
    ...
    def _assert_matching_entities_to_render(self):
        assert (
            self._sensor_depth.get_entity_to_render()
    ...
    def _assert_matching_window_sizes(self):
        assert (
            self._sensor_depth.get_windowed_size()
    ...
    def _create_uniform_pixel_coords_image(image_dims):
        pixel_x_coords = np.reshape(
            np.tile(np.arange(image_dims[1]), [image_dims[0]]),
            (image_dims[0], image_dims[1], 1),
        ).astype(np.float32)
        pixel_y_coords_ = np.reshape(
            np.tile(np.arange(image_dims[0]), [image_dims[1]]),
    ...
    def _get_planar_to_radial_depth_scalars(self):
        ...
        # polar coord image
        coord_img = self._create_uniform_pixel_coords_image(
            [self._omni_resolution[1], self._omni_resolution[0]]
        )
        ...
        pos_x = np.logical_and(
            xy_region,
            np.logical_or(phi <= 45 * np.pi / 180, phi >= (45 + 270) * np.pi / 180),
        )
        pos_y = np.logical_and(
            xy_region,
            np.logical_and(phi >= 45 * np.pi / 180, phi <= (45 + 90) * np.pi / 180),
        )
        neg_x = np.logical_and(
            xy_region,
            np.logical_and(
                phi >= (45 + 90) * np.pi / 180, phi <= (45 + 180) * np.pi / 180
            ),
        )
        neg_y = np.logical_and(
            xy_region,
            np.logical_and(
                phi >= (45 + 180) * np.pi / 180, phi <= (45 + 270) * np.pi / 180
            ),
        )
        ...
        return np.where(
            pos_x,
            recip_sin_theta_cos_phi,
            np.where(
                neg_x,
                -recip_sin_theta_cos_phi,
                np.where(
                    pos_y,
                    recip_sin_theta_sin_phi,
                    np.where(
                        neg_y,
                        -recip_sin_theta_sin_phi,
                        np.where(
                            ...
                            recip_cos_theta,
                            np.where(
                                neg_z, -recip_cos_theta, np.zeros_like(recip_cos_theta)
                            ),
                        ),
                    ),
                ),
    ...
    def handle_explicitly(self) -> None:
        ...
        without PyRep.step().
        """
        SimBackend().sim_vision_api.handleSpherical(
            self._sensor_rgb._handle,
            self._six_sensor_handles,
            360,
            180,
            self._sensor_depth._handle,
        )
    ...
    def capture_depth(self, in_meters=False) -> np.ndarray:
        ...
        """
        planar_depth = (
            self._sensor_depth.capture_rgb()[:, :, 0] * self._clipping_plane_diff
    ...
    def set_entity_to_render(self, entity_to_render: int) -> None:
        ...
        This can be an object or more usefully a collection.
