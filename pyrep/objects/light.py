...
class Light(Object):
    ...
    def set_specular(self, specular):
        ...
        self._sim_api.setLightParameters(
            self._handle, self.is_on(), None, list(self.get_diffuse()), list(specular)
        )
    ...
    def get_intensity_properties(self):
        ...
        :return: The light intensity properties cast_shadows,
            spot_exponent, spot_cutoff, const_atten_factor,
        linear_atten_factor, quad_atten_factor
        """
        cast_shadows = self._sim_api.getObjectInt32Param(
            self._handle, simc.sim_lightintparam_pov_casts_shadows
        )
        spot_exponent = self._sim_api.getObjectFloatParam(
            self._handle, simc.sim_lightfloatparam_spot_exponent
        )
        spot_cutoff = self._sim_api.getObjectFloatParam(
            self._handle, simc.sim_lightfloatparam_spot_cutoff
        )
        const_atten_factor = self._sim_api.getObjectFloatParam(
            self._handle, simc.sim_lightfloatparam_const_attenuation
        )
        linear_atten_factor = self._sim_api.getObjectFloatParam(
            self._handle, simc.sim_lightfloatparam_lin_attenuation
        )
        quad_atten_factor = self._sim_api.getObjectFloatParam(
            self._handle, simc.sim_lightfloatparam_quad_attenuation
        )
        return (
            bool(cast_shadows),
            spot_exponent,
    ...
    def set_intensity_properties(
        ...
        :param const_atten_factor: light constant attenuation factor,
            currently not supported.
        :param linear_atten_factor: light linear attenuation factor,
            currently not supported.
        :param quad_atten_factor: light quadratic attenuation factor,
            currently not supported.
        """
        if cast_shadows is not None:
            self._sim_api.setObjectInt32Parameter(
                self._handle,
                simc.sim_lightintparam_pov_casts_shadows,
                int(cast_shadows),
            )
        if spot_exponent is not None:
            if spot_exponent % 1 != 0:
                warnings.warn(
                    "spot exponent must be an integer, rounding input of "
                    f"{spot_exponent} to {round(spot_exponent)}"
                )
            self._sim_api.setObjectFloatParameter(
                self._handle,
                simc.sim_lightfloatparam_spot_exponent,
        ...
        if spot_cutoff is not None:
            ...
            if spot_cutoff > np.pi / 2:
                warnings.warn(
                    f"Tried to set spot_cutoff to {spot_cutoff}, but the maximum "
                    "allowed value is pi/2, therefore setting to pi/2"
                )
            self._sim_api.setObjectFloatParameter(
