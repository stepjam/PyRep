...
class VisionSensor(Object):
    ...
    def create(
        ...
        show_volume_not_detecting=True,
        show_volume_detecting=True,
        passive=False,
        ...
        int_params = [resolution[0], resolution[1], 0, 0]  # 0  # 1  # 2  # 3
    ...
    def handle_explicitly(self) -> None:
        ...
        if not self.get_explicit_handling():
            raise RuntimeError(
                "The explicit_handling is disabled. "
    ...
    def capture_depth(self, in_meters=False) -> np.ndarray:
        ...
        """
        enc_img, resolution = self._sim_api.getVisionSensorDepth(
            self._handle, int(in_meters)
        )
        img = np.frombuffer(enc_img, dtype=np.float32).reshape(
    ...
    def get_intrinsic_matrix(self):
        ...
        return np.array(
            [
                [focal_lengths[0], 0.0, pp_offsets[0]],
                [0.0, focal_lengths[1], pp_offsets[1]],
                [0.0, 0.0, 1.0],
            ]
        )
    ...
    def get_perspective_mode(self) -> PerspectiveMode:
        ...
        perspective_mode = self._sim_api.getObjectInt32Param(
            self._handle,
            simc.sim_visionintparam_perspective_operation,
        )
    ...
    def set_render_mode(self, render_mode: RenderMode) -> None:
        ...
        """
        self._sim_api.setObjectInt32Param(
            self._handle, simc.sim_visionintparam_render_mode, render_mode.value
        )
    ...
    def set_windowed_size(self, resolution: Sequence[int] = (0, 0)) -> None:
        ...
        """
        self._sim_api.setObjectInt32Param(
            self._handle, simc.sim_visionintparam_windowed_size_x, resolution[0]
        )
        self._sim_api.setObjectInt32Param(
            self._handle, simc.sim_visionintparam_windowed_size_y, resolution[1]
        )
    ...
    def get_perspective_angle(self) -> float:
        ...
        """
        return math.degrees(
            self._sim_api.getObjectFloatParam(
    ...
    def set_perspective_angle(self, angle: float) -> None:
        ...
        self._sim_api.setObjectFloatParam(
            self._handle,
            simc.sim_visionfloatparam_perspective_angle,
            math.radians(angle),
        )
    ...
    def set_entity_to_render(self, entity_to_render: int) -> None:
        """Set the entity to render to the Sensor.
    ...
    def get_entity_to_render(self) -> int:
        """Get the entity to render to the Sensor.
