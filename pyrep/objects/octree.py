...
class Octree(Object):
    ...
    @staticmethod
    def create(
        voxel_size: float, point_size: Optional[float] = None, options: int = 0
    ) -> "Octree":
    ...
    def insert_voxels(
        self, points: List[float], color: Optional[list[float]] = None, options: int = 0
    ) -> None:
        ...
        if not isinstance(points, list):
            raise ValueError("Octree.insert_voxels: points parameter is not a list.")
        if len(points) % 3 != 0:
            raise ValueError(
                "Octree.insert_voxels: points parameter length " "not a multiple of 3."
            )
        if color is not None:
            if not isinstance(color, list):
                raise ValueError("Octree.insert_voxels: color parameter not a list.")
            elif len(color) != 3:
                raise ValueError(
    ...
    def remove_voxels(self, points: Optional[List[float]], options: int = 0) -> None:
    ...
    def insert_object(
        self, obj: Object, color: Optional[list[float]] = None, options: int = 0
    ) -> None:
        ...
        if color is not None:
            if not isinstance(color, list):
                raise ValueError("Octree.insert_object: color parameter not a list.")
            elif len(color) != 3:
                raise ValueError(
                    "Octree.insert_object: color parameter not an RGB list."
                )
        self._sim_api.insertObjectIntoOctree(
            self._handle, obj.get_handle(), options, color, 0
        )
        return
    ...
    def check_point_occupancy(self, points: List[float], options: int = 0) -> bool:
        if not isinstance(points, list):
            raise ValueError(
        ...
        res, tag, loc_low, loc_high = self._sim_api.checkOctreePointOccupancy(
            self._handle, options, points
        )
        return bool(res)
