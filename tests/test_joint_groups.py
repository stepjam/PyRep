...
class TestJointGroups(TestCore):
    ...
    def test_get_joint_types(self):
        self.assertEqual(
            self.robot.get_joint_types(), [JointType.REVOLUTE] * self.num_joints
        )
    ...
    def test_get_set_joint_target_positions(self):
        ...
        self.assertEqual(len(self.robot.get_joint_target_positions()), self.num_joints)

    def test_get_set_joint_target_velocities(self):
        self.robot.set_joint_target_velocities([0.1] * self.num_joints)
        self.assertEqual(len(self.robot.get_joint_target_velocities()), self.num_joints)
    ...
    def test_get_velocities(self):
        self.assertEqual(len(self.robot.get_joint_velocities()), self.num_joints)
